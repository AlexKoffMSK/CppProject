...
namespace GameMatrix
{
	...
	struct Point
	{
		...
		Point() = default; //(либо Point(){};

		//Point(int x_, int y_)
		//{						//здесь уже появляется некий _x и _y со значением 0, т.к. 33 и 34 строки следует читать как: инициализировать нулем, если не будет другой инициализации
		//	_x = x_;			//это не инициализация _x, а переписывание значения _x
		//	_y = y_;			//это не инициализация _y, а переписывание значения _y
		//						//при таком алгоритме - сначала появляется _x и _y со значение 0 (33,34 строки), а потом переписывается на значение, указанное в конструкторе x_ и y_
		//}

		Point(int x_, int y_)
			: _x (x_)			//инициализация _x до момента появления _x со стартовым значением x_
			, _y (y_)			//инициализация _y до момента появления _x со стартовым значением y_
								//при таком алгоритме - _x и _y сразу инициализируются x_ и y_, а не переписываются из стартовых значений (_x=0, _y=0)
		{
		}
